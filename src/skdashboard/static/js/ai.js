...
function laneCard(spec, source) {
  ...
  return `<article class="ai-lane" data-lane="${esc(spec.id)}"><h3>${esc(spec.label)}</h3><p>${esc(source.truth_state)} | ${esc(coverage(source))}</p><dl class="ai-measures"><div><dt>Tokens</dt><dd>${value(source, "tokens_total", "tokens")}</dd></div><div><dt>Estimated cost</dt><dd>${value(source, "cost_usd", "USD")}</dd></div><div><dt>Cost state</dt><dd>${source.aggregate ? esc(source.aggregate.cost_state) : '<span class="ai-unknown">Unknown</span>'}</dd></div><div><dt>Latency</dt><dd>${value(source, "latency_ms", "ms")}</dd></div><div><dt>Cache ratio</dt><dd>${source.aggregate && source.aggregate.cache_ratio != null ? `${esc((Number(source.aggregate.cache_ratio) * 100).toFixed(1))}%` : '<span class="ai-unknown">Unknown</span>'}</dd></div><div><dt>Denials</dt><dd>${value(source, "denial_count", "requests")}</dd></div><div><dt>Errors</dt><dd>${value(source, "error_count", "errors")}</dd></div><div><dt>Pricing revision</dt><dd class="ai-unknown">Not projected</dd></div><div><dt>Cost confidence</dt><dd class="ai-unknown">Not reported</dd></div><div><dt>Latency</dt><dd class="ai-unknown">Not projected</dd></div><div><dt>Quality</dt><dd class="ai-unknown">Unknown</dd></div><div><dt>Value</dt><dd class="ai-unknown">Unknown</dd></div></dl></article>`;
}
