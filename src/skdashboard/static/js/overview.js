...
function renderOperatorBrief(items, quality) {
  ...
  const cards = [
    `<div><dt>Current conditions</dt><dd>${usable.length} authorized aggregate source${usable.length === 1 ? "" : "s"} available; no causal claim is made.</dd></div>`,
    `<div><dt>Risks and anomalies</dt><dd>${risks.length ? risks.map((item) => `${esc(item.adapter_id)} is ${esc(item.truth_state)}`).join("; ") : "No source freshness or coverage risk detected."}</dd></div>`,
    `<div><dt>Ranked next-step proposals</dt><dd>${ranked.length ? ranked.map((item, i) => `${i + 1}. Inspect ${esc(item.adapter_id)} evidence before any proposal`).join("; ") : "1. Continue read-only monitoring; no action is proposed."}</dd></div>`,
